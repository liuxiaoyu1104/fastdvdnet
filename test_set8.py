...
from utils.utils import batch_psnr, init_logger_test, \
...
# 	print(torch.load(args['model_file'], map_location=device)['training_params'])
# 	print(torch.load(args['model_file'], map_location=device)['args'])
	if args['cuda']:
		device_ids = [0]
	...
	# for name in model_temp.state_dict():
	# 	print("{:30s}:{}".format(name,model_temp.state_dict()[name].shape))

	model_temp.eval()
	seqs_dirs = sorted(glob.glob(os.path.join(args['test_path'], VALSEQPATT)))
	last_psnr,count=0,0
	for seq_dir in seqs_dirs:
		with torch.no_grad():
			# process data
			
			seq, _, _ = open_sequence(seq_dir,\
										args['gray'],\
...
if __name__ == "__main__":
	...
	parser.add_argument("--model_file", type=str,\
						default="./log/logs_only_dataset_shixi/ckpt_e40.pth", \
						help='path to model of the pretrained denoiser')
	parser.add_argument("--test_path", type=str, default='/home/mist/dataset/DAVIS/test', \
	...
	parser.add_argument("--max_num_fr_per_seq", type=int, default=85, \
						help='max number of frames to load per sequnce')
	parser.add_argument("--noise_sigma", type=float, default=40, help='noise level used on test set')
	parser.add_argument("--dont_save_results",default=True , help="don't save output images")
