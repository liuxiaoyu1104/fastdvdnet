...
from utils.utils import svd_orthogonalization, close_logger, init_logging, normalize_augment
...
def main(**args):
	...
	for epoch in range(start_epoch, args['epochs']):
		...
		for i, data in enumerate(loader_train, 0):
			...
			#在这里做的调整 为了保证训练的时候图像的大小一致 我现在随机裁成144 然后缩放后再裁成96

			seq = data[0]['data']
			n,t,c,h, w = seq.shape            
			seq_scale=torch.zeros([n,t,c,args['patch_size'],args['patch_size']])
			for j in range(seq.shape[0]):
				aug = np.random.randint(0, 4)
				scale=scales[aug]
				seq_scale_single=F.interpolate(seq[j], size=[int(h*scale),int(w*scale)],mode='bicubic')
				# print(seq_scale_single.shape)
				T,C,H,W=seq_scale_single.shape
				position_h = np.random.randint(0, H-args['patch_size']+1)
				position_w = np.random.randint(0, W-args['patch_size']+1)
				seq_scale[j] = seq_scale_single[:, :, position_h:position_h+args['patch_size'], position_w:position_w+args['patch_size']]
			# print(seq_scale.shape)
			img_train, gt_train = normalize_augment(seq_scale.cuda(), ctrl_fr_idx)
...
if __name__ == "__main__":
	...
	parser.add_argument("--max_number_patches", "--m", type=int, default=384000, \
						help="Maximum number of patches")
	# Dirs
	parser.add_argument("--log_dir", type=str, default="./log/tui_scale", \
					 help='path of log files')
	parser.add_argument("--trainset_dir", type=str, default='/hdd/dataset/fastdvd/train_seq/mp4', \
					 help='path of trainset')
	parser.add_argument("--valset_dir", type=str, default='/hdd/dataset/fastdvd/test_sequences/DAVIS-Test/', \
